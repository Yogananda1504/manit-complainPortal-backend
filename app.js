...
import { keycloak, sessionConfig } from "./middleware/sso.js";
...
app.use((req, res, next) => {
  // Extract IP address (if x-forwarded-for contains multiple IPs, take the first one)
  const forwarded = req.headers["x-forwarded-for"];
  const ip = forwarded ? forwarded.split(",")[0].trim() : req.connection.remoteAddress;

  const geo = geoip.lookup(ip);
  // Allow request if geo lookup returns null or if the country is India ("IN")
  if (!geo || geo.country === "IN") {
    return next();
  }
  // Deny access for other countries
...
// Apply session middleware before Keycloak
